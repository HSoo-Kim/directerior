...
name: directerior
description: >-
  Experiment directory manager with HDD offload. MUST USE for new experiments, organizing experiment outputs, piled-up or oversized results, adopting outputs from unexpected paths, status, restore, or cleanup. Creates configurable method/experiment {code,results} hierarchies, scans size thresholds, and only after explicit user approval moves results to HDD and junction-links them back. Never rewrites source code, imports, or agent instruction files. Do not initialize for one-off analysis, document review, presentation, report, or other non-experiment deliverables. Triggers: directerior, new experiment, organize experiments, experiment directory, offload results, move results to HDD, restore results, adopt experiment outputs, experiment status, clean experiment dirs, 실험 디렉토리, 실험 정리, 실험 결과물 HDD 이동, 방법론별 실험 관리.
---
