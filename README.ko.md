...
`add`가 다음 접두사를 자동 계산합니다.

에이전트가 Directerior의 고정 실험 컨테이너 밖에 디렉토리를 만들 때는 모든
계층에 숫자 순서 접두사와 목적을 드러내는 이름을 사용합니다. 번호는 부모마다
다시 시작합니다. 예: `0_reference/`,
`1_analysis/1_functional-states-manuscript-review/`, `1_validation/`.
디렉토리 이름은 사용한 도구가 아니라 연구 활동이나 산출물의 목적을 나타내야
하며, `work`, `misc`, `outputs`처럼 모호한 이름은 피합니다. 한 요청에서 나온
원본 사양, 결과물, 검증 자료는 모두 그 요청의 번호가 붙은 디렉토리 아래에
둡니다.

Directerior 자체는 실험과 실험 결과물용입니다. 논문 검토, 일회성 분석, 보고서,
발표자료, 문서 산출물을 관리하려고 가짜 방법론이나 `expNN-*` 디렉토리를 만들지
않습니다.
